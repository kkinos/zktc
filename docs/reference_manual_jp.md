...
ビット 2 はハードウェア割り込みが発生すると 1 がセットされ、ビット 1 はソフトウェア割り込み(トラップ命令)により 1 がセットされます。ビット 0 は 1 の時割り込みがマスク(無効化)されます。ビット 0 は割り込みが発生すると 1 にセットされます。詳しくは[例外](#例外)を参照してください。
...
例外にはリセット、ソフトウェア割り込み(トラップ命令)、ハードウェア割り込みの 3 種類あります。リセットは常に実行されますが、ソフトウェア割り込みやハードウェア割り込みは PSR のビット 0 が 1 にセットされている場合はマスクされます。
