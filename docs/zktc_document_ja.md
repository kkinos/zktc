ZKTC は SystemVerilog で実装された 16 ビット CPU です。FPGA で動かすことができます。システムを構成する周辺機能として UART、GPIO を実装しています。割り込みにも対応しています。
...
命令中に即値(Immediate)を 5 ビット取る命令です。
...
命令中に即値(Immediate)を 8 ビット取る命令です。
...
1. 未定義命令の次の命令を指している PC を PPC、PSR を PPSR にコピーします。
...
1. トラップ命令の次の命令を指している PC を PPC、PSR を PPSR にコピーします。
...
ZKTC にはペリフェラルとして LED、UART、GPIO があり、 MMIO で操作することができます。
...
| 0x8102  | UART の送信開始 (下位 1 ビット) <br>0 : not start 1 : start                                     | W   |
